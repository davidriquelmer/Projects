...
class MultiLang:
    ...
    """
    def __call__(self): self._delegate()
    ...
    def processor(self) -> spacy.language.Language:
        ...
        try:
            model_name = {
                ...
                "de": "de_core_news_sm",
                "ru": "ru_core_news_sm",
            }.get(self.lang.lang, self.lang.lang)  # Use default to handle full model names
            return spacy.load(model_name, disable=['ner']) if self.enable_wiki_ner else spacy.load(model_name)
        except IOError as e:
            print(f"Error: Unable to load model '{model_name}', falling back to blank model.")
            return spacy.blank(self.lang.lang)
    ...
    def ner(self) -> spacy.language.Language:
        ...
        try:
            return spacy.load('en_core_web_sm') if self.enable_wiki_ner else None
        except IOError:
            print("Error: Unable to load multilingual NER model, falling back to blank model.")
            return None
